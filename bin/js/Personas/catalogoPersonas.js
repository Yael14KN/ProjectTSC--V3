...
(function (ModuloPersona) {
    ...
    class clsPersona {
        ...
        met_agregar() {
            let _inputNombre = d3.select("#txtNombre").property("value");
            let _inputEstatus = d3.select("#txtEstatus").property("value");
            let _inputGenero = d3.select("#txtGenero").property("value");
            if (_inputNombre !== "" && _inputEstatus !== "" && _inputGenero !== "") {
                ...
                if (this._esEditar == true) {
                    this._texto = "Agregar";
                    let _actualizar = this._map.get(this._obj.id);
        ...
        met_imprimir() {
            ...
            let _filtroTelefono = d3
                .select("#txtFiltroTelefono")
                .property("value");
            if (_filtro !== "" || _filtroTelefono !== "") {
                this._litaTemporal = this._litaTemporal.filter((a) => {
                    let _f1 = true;
                    if (_filtro !== "")
            ...
            if (this._tipoOrden !== Tipo.SinOrden) {
                if (this._tipoOrden == Tipo.Ascendente)
                    this._litaTemporal.sort((a, b) => d3.ascending(a.nombre, b.nombre));
                else if (this._tipoOrden == Tipo.Descendente)
                    this._litaTemporal.sort((a, b) => d3.descending(a.nombre, b.nombre));
            }
        ...
        met_tabla() {
            let _fila = d3
                .select("tbody")
                .selectAll(".fila")
                .data(this._litaTemporal, (a) => a.id);
            _fila.join((enter) => {
                let _celda = enter
                    .append("tr")
                    .attr("class", "fila")
        ...
        met_eliminar(id) {
            if (this._map.has(id.id))
                this._map.delete(id.id);
            this.met_imprimir();
            alert("Eliminado");
        }
        met_obtener(personaje) {
            this._esEditar = true;
    ...
    ModuloPersona.clsPersona = clsPersona;
})(ModuloPersona || (ModuloPersona = {}));
//# sourceMappingURL=catalogoPersonas.js.map